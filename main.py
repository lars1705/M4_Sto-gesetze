...
def mittelwert_t1(header:str, y_0:float, header_2:str, path:str="daten1.csv", c:int=1):
    ...
    for i in range(5):
        ...
        m_werte1.append(ufloat(uncertainties.nominal_value(m_wert), standartabweichung_mwert_mit_tverteilung(
            varianz([uncertainties.nominal_value(x) for x in auslenkungen_res]), len(auslenkungen_res))))
...
def mittelwert_t2(header:str="Auslenkung 2 (resultierend) T2", path:str="daten2.csv"):
    ...
    for i in range(6):
        ...
        #print(auslenkungen_res)
        m_wert = sum(auslenkungen_res) / len(auslenkungen_res)
        m_werte2.append(ufloat(uncertainties.nominal_value(m_wert), standartabweichung_mwert_mit_tverteilung(
            varianz([uncertainties.nominal_value(x) for x in auslenkungen_res]), len(auslenkungen_res))))
    print("\n>>>>>>>>>>>>",m_werte2)
...
def plot_ausgleichsgerade(x_name:str, mittelwerte_y:list, x_0:float, c:int, path:str="daten1.csv"):
    ...
    plt.plot(x1_nominal, f(x1_nominal, unumpy.nominal_values(m_fit), unumpy.nominal_values(n_fit)), 'r-',
            label=rf'Ausgleichsgerade: {m_fit} $\cdot$ $a_1$  + {n_fit} [cm]')
    plt.tick_params(axis='both', labelsize="35")
    plt.xlabel(r'Auslenkung $a_1$ [cm]', fontsize="35")
    plt.ylabel(r"Auslenkung $a_2'$ (resultierend) [cm]", fontsize="35")
...
def plot_rollende_Kugel(path="daten2.csv", show_figure=True):
    ...
    h_s = [(get_h(h_0=h_0, s=s, L=L, H=H)*100)**(1/2)  for s in fallhöhen]  #in sqrt cm
    print(h_s)
    ...
    plt.plot(hs_nom, f(hs_nom, unumpy.nominal_values(m_fit), unumpy.nominal_values(n_fit)), 'r-',
            label=rf"Ausgleichsgerade: ({m_fit} $\cdot$"+"$\sqrt{h}$ "+f"- {abs(n_fit)}) [cm]")
    plt.tick_params(axis='both', labelsize="35")
...
def varianz(werte:list) -> float:
    print(werte)
    for i, x in enumerate(werte):
        if type(x) == uncertainties.core.AffineScalarFunc:
            werte[i] = uncertainties.nominal_value(x)
    mittelwert = np.mean(werte)
    sum_quad_abweichung = 0
    for x in werte:
        sum_quad_abweichung += (x-mittelwert)**2
    print(sum_quad_abweichung)
    return np.sqrt(sum_quad_abweichung/(len(werte)-1))


def standartabweichung_mwert_mit_tverteilung(varianz:float, n, t:float=1.14):
    return varianz * 1/(np.sqrt(n)) * t
...
#
    print(get_epsilon(m=plot_rollende_Kugel(show_figure=True), l=l))
    #df = pd.read_csv("daten1.csv", delimiter=",")
    #for column in df:
    #    df[column] = df[column].str.replace(",", ".").astype(float)
    #v = varianz(list(df["Auslenkung 2 (resultierend) T1"][:5:].values))
    #print(v)
